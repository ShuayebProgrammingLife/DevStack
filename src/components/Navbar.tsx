...
export default function Navbar() {
  ...
  return (
    <header className="sticky top-0 z-50 border-slate-100 bg-white/90 backdrop-blur" >
      <div className="mx-auto flex h-16 max-w-7xl items-center justify-between px-4 sm:px-6 lg:px-8" >
        {/* for phn humburger left side */}
        <button type="button" aria-label="Toggle navigation menu" aria-expanded={open}
          onClick={() => setOpen((v) => !v)}
          className="grid h-9 w-9 place-items-center rounded-md text-state-700 hover:bg-slate-100 md:hidden" >
        ...
        <nav className="hidden items-center gap-8 md:flex">{NAV_LINKS.map((link, i) => (
          <a
            ...
            className={`text-sm font-medium transition-colors hover:text-slate-900 ${i === 0 ? 'text-pink-600' : 'text-slate-600'
              }`}
          >
            {link}
          </a>
                  ...
                  <li key={link}>
                    <a
                      ...
                      className={`block rounded-md px-2 py-2 text-sm font-medium ${
                        i===0 ? 'text-pink-600' : 'text-slate-700'
                      } hover:bg-state-50`}
                      >
                        {link}
                      </a>
